    ...
    for item in one_batch:
        for key in keys:
            if key == 'labels':
                ...
                preprocessed = item[key]
                if preprocess_fn is not None:
                    # 如果preprocess_fn存在，则对inputs进行预处理
                    preprocessed = [preprocess_fn(k) for k in item[k]]
                cache_batch.setdefault(key, []).append(preprocessed)
    # 编码inputs：{'input_ids': tensor, 'token_type_ids': tensor, 'attention_mask': tensor}
...
class DataIterator:
    ...
    def from_corpus(cls,
                    ...
                    is_split_into_words: Optional[bool]=None, # 在bert调用tokenizer时传入
        ...
        if config['task_tag'] == 'ner':
            # 如果config的任务类型为ner任务，map_func使用map_func_for_ner
            map_func_core = map_func_for_ner
        else:
            # 如果config的任务类型为其他任务，map_func使用map_func_basic
            map_func_core = map_func_basic
        if is_split_into_words is None:
            # is_split_into_words是tokenizer的一个参数，如果是None，则需要进行设置
            if config['task_tag'] == 'ner':
                # 如果任务类型是ner，则设置为True
