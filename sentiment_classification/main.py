...
parser.add_argument('--corpus_tag_or_dir', type=str, default='./data/weibo_senti_100k/')
...
model = AutoModelForSequenceClassification.from_pretrained(
    pretrained_model_name_or_path=con.pretrain_model_path,
    num_labels=con.num_labels # 标签数量
)
# 将模型设置为训练模式
model.train()
# 将模型拷贝到设备上
...
# 初始化优化器
optimizer = torch.optim.AdamW(model.parameters(), lr=con.learning_rate)
...
def train_with_eval():
    ...
    for train_step in range(total_train_steps):
        ...
        with autocast():
            ...
            trian_output = model(**train_data)
            # 得到loss
            train_loss = trian_output['loss']
